...
class SimpleSelfAttention(nn.Module):
    ...
    def forward(self, x):
        # 获取输入的形状, N: 批次大小, seq_len: 序列长度， x shape: [N, seq_len, embed_size]
        N, seq_len, _ = x.shape
...
class LightPositionalEncoding(nn.Module):
    def __init__(self, embed_size, max_len=5000, scale=0.1):
        ...
        position = torch.arange(0, max_len, dtype=torch.float).unsqueeze(
            1)  # 得到一个shape为(max_Len, 1)的张量，内容为0-max_len-1
        ...
        # 将位置编码注册为模型参数，这样在训练过程中就不需要更新了, 同时在前面加一个维度，方便和输入的形状匹配
...
class ImprovedTransformerRegressor(nn.Module):
    def __init__(self, seq_len=10, embed_size=32, num_layers=2, heads=4):
        ...
        self.layers = nn.ModuleList(
            [SimpleTransformerBlock(embed_size, heads)
...
def generate_improved_data(num_samples=5000, seq_len=10, max_value=10):
    ...
    # 最大可能的累积和，这样Y最后的范围也被归一化到[0, 1]的范围
    Y_normalized = Y / (max_value * seq_len)

    print(f"数据统计:")
    print(
        f"X范围: [{X.min():.2f}, {X.max():.2f}] -> 归一化: [{X_normalized.min():.3f}, {X_normalized.max():.3f}]")
    print(
        f"Y范围: [{Y.min():.2f}, {Y.max():.2f}] -> 归一化: [{Y_normalized.min():.3f}, {Y_normalized.max():.3f}]")
...
if __name__ == "__main__":
    ...
    model = ImprovedTransformerRegressor(
        seq_len, embed_size, num_layers, heads)
    ...
    for epoch in range(1000):
        ...
        if epoch % 50 == 0:
            ...
            print(
                f"Epoch {epoch:03d} | Train={loss.item():.4f} | Test={test_loss.item():.4f}")
    ...
    sample_x_original = torch.tensor(
        [[1, 0, 3, 4, 0, 2, 3, 1, 1, 0]], dtype=torch.float32)
    ...
    # ✅ 修正：反归一化得到真实预测值
    pred_y_denormalized = pred_y_normalized * \
        (max_value * seq_len)  # 乘以最大可能累积和
